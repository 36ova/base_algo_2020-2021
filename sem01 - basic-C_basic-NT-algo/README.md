...
* `<<` Это оператор побитового сдвига. Грубо говоря, *направляет* то, что вы хотите вывести в поток вывода (консоль).
...
int main() {
  int a; //type of x is required, in other case verdict is Compilation error (CE)
...
Цикл while чаще всего используется для того, чтобы дельть нечто, пока выражение истинно. Его общий вид:
...
Как видите, данный код объемнее, чем с циклом *for*, но это не значит, что он всегда удобнее. В дальнейшем мы столкнемся с случаем, когда понятнее цикл *while*.
...
Факторизация - процесс разложения натурального числа на простые множители в соответствующиим степенями. Основная теорема арифметики утверждает, что для любого натурального числа такое разложение существует и единственно. Например, *36 = 2 * 2 * 3 * 3*. Суть алгоритма в переборе чисел от 2 до n и если число является делителем, то выписываем его. Сложность такого алгоритма линейная, так как число может оказаться уже простым. Поэтому стоит заметить факт из школьного курса математики, что делители ходят парами, то есть если простое число p входит в разложение, то оно не может быть больше, чем корень из числа, округленный вверх. Поэтому достаточно перебирать не от 2 до n, а от 2 до корня из n.
